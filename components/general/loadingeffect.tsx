...
const BlurText: React.FC<BlurTextProps> = ({
  ...
  const springs = useSprings(
    ...
    elements.map((_, i) => ({
      ...
      to: async (next: (arg: Record<string, SpringValue<any>>) => Promise<void>) => {
        while (true) {
    ...
    <p ref={ref} className={`blur-text ${className} flex flex-wrap`}>
      {springs.map((props, index) => (
        <AnimatedSpan
          key={index}
          style={props}
          className="inline-block text-[50px] font-bold font-serif text-white transition-transform will-change-[transform,filter,opacity]"
        >
